                ...
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="card-title">
                        <h3 class="card-label">
                            ...
                            <span class="d-block text-muted pt-2 font-size-sm">Actualiza el nombre y permisos de rol</span>
                        </h3>
                    </div>
			    </div>
                
				<div class="card-body">
					{!! Form::model($role, ['data-return' => route('admin.roles.index'),'class'=>'roles-form','id'=>'roles-form','route'=>['admin.roles.update',$role],'method'=>'put']) !!} 

                         {{ csrf_field() }}
                             <div class="form-group">
                            {!! Form::label ('name','Nombre') !!}
                            {!! Form::text ('name',null,['class'=>'form-control','placeholder'=>'','autofocus']) !!}
                            {!! Form::text ('id',null,['hidden','readonly']) !!}
					    ...
					    <div class="form-group">
						    ...
						    <div class="checkbox-list">
						        ...
						        <div class="card-footer">
							        {!! Form ::submit('Actualizar',['class'=>'btn btn-primary mx-1']) !!}
                    ...
                    .then(function (response) {
                        ...
                        if (data.errors) {
                            $.each(data.errors, function(key, value){
                                ...
                                $('.alert-errores').removeClass('d-none');
                                $('.alert-errores').append(`<p>${data.existe}</p>`);                          
                        }else if (data.norol) {
                                $('.alert-errores').removeClass('d-none');
                                $('.alert-errores').append(`<p>${data.norol}</p>`);                          
                        }
                        else {
                            Swal.fire({
                                title: "Actualizado",
